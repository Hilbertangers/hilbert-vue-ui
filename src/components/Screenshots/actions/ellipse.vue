  ...
  methods: {
    ...
    mousedown(e, { el, ctx, context }) {
      ...
      if (!this.inStroke.is) {
        ...
        // 拖拽准备
        if (this.todo === 'drag') {
          this.setContext({ cursor: 'grabbing' })
    ...
    mousemove(e, { el, ctx, context }, pointInStroke) {
      ...
      if (this.isNew) {
        ...
        if (action) { // 鼠标触摸路径
          ...
          if (this.isEdit) {
            ...
            if (hit) {
              this.todo = 'resize'
              this.setContext({ cursor: hit.cursor })
          ...
          this.todo = null
          this.setContext({ cursor: 'crosshair' })
        }
      }
    },
