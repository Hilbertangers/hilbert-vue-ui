  ...
  **/
  ...
  export default {
    ...
    props: {
      ...
      nodeMap: { // node节点映射表，如[{value: '1', label: '昂热'}]，node显示为`1:昂热`
        type: Array,
        default: () => [],
      },
    },
    ...
    mounted() {
      ...
      this.render();
      this.observer.listenTo(this.$refs.wrapper, this.draw);
    },
    watch: {
      value() {
        this.render();
    ...
    methods: {
      render() {
        ...
        this.$emit('fetch-struct', this.struct);
      ...
      setWidth() {
        let width = this.reactiveWidth || this.$refs.wrapper.offsetWidth;
        this.canvas.width = width;
        return width;
      },
      setHeight() {
        let minBranchH = this.nodeStyle.height + this.nodeStyle.branchPaddingMin;
      ...
      drawStruct(ctx, struct, parentW, parentH, parentX, parentY, gradientRate) {
        ...
        if (scene === 'justOr') {
          let w = parentW * this.orWidthRate / struct.length;
          let h = parentH * this.orHeightRate;
          let y = (parentH - h) / 2 + parentY;

          gradientRate.push(this.orWidthRate / struct.length);
        ...
        if (scene === 'justNode') {
          ...
          if (margin < this.nodeStyle.marginMin) {
            let distance = (this.nodeStyle.marginMin - margin) * (struct.length + 1);
        ...
        if (scene === 'orAndNode') {
          ...
          let orW = parentW * this.orHybridRate;
          let singleOrW = orW / orArr.length;
          let margin = (parentW - (nodeW + orW)) / (nodeArr.length + orArr.length + 1);

          if (margin < this.nodeStyle.marginMin) {
            let distance = (this.nodeStyle.marginMin - margin + 50) * (struct.length + 1);
            this.setReactiveWidth(gradientRate, distance + parentW);
            return 'restart';
          }
      ...
      drawNode(ctx, node, initX, initY) {
        let { height, color } = this.nodeStyle;
        ...
        ctx.font = `${this.nodeStyle.fontSize}px sans-serif`;
      ...
      fillNode(value) {
        const { padding, fontSize } = this.nodeStyle;
        const _ctx = this.canvas.getContext('2d');
        _ctx.font = `${fontSize}px sans-serif`;

        const match = this.nodeMap.find(_t => _t.value === value);
    ...
    beforeDestroy () {
      this.observer.uninstall(this.$refs.wrapper);
    },
  };
</script>
